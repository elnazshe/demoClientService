    ...
    public ResponseDto<MenuItmDto> getUserMenu() {
        MenuItmDto menuItmDto = new MenuItmDto(null, null, null, new ArrayList<MenuItmDto>(Arrays.asList(
                new MenuItmDto(MenuItemType.MENU, "امور کابران", null, new ArrayList<MenuItmDto>(Arrays.asList(
                        ...
                        new MenuItmDto(MenuItemType.PAGE, "جستجو", new UIPageDto(null, "add.xml"), new ArrayList<MenuItmDto>()))))
        )));
...
//        return new ResponseDto(ResponseStatus.Ok, "", "ثبت با موفقیت انجام شد", null);
//        return new ResponseDto(ResponseStatus.Error, "", "", new ResponseException("طلاعات درست وارد نشده"));
        return new ResponseDto(ResponseStatus.Ok, "", "اطلاعات ذخیره شد.", null);
    }

    @RequestMapping(value = "/ws/search", method = RequestMethod.POST)
    public ResponseDto<Person> serach(@RequestParam String nationalCode) {
    ...
    public ResponseDto<Person> serachAll(@RequestParam String name) {
        for (Person person : personList) {
